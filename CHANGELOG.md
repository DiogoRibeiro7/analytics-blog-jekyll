...
- Figures can carry a dark companion. Put `power-dark.png` beside `power.png` and it becomes a `<source media="(prefers-color-scheme: dark)">` at the front of the same `<picture>`, with its own resized copies and modern formats; the `<img>` keeps the light file, its alt text and its dimensions, so the figure is described once. `{% figure %}` takes `dark_src` for a companion that does not follow the convention, as does any `<img data-dark-src="...">`, including one the variant pipeline never reads, such as an SVG. `theme_options.images.dark_suffix` changes the suffix, and `""` turns the convention off. `prefers-color-scheme` follows the operating system and the theme's toggle does not, so `core/dark-mode.js` overrules the query on those sources whenever a reader has chosen a mode for the site; with JavaScript off the figures follow the system. A page's `hero_image` gets the same treatment, through `components/responsive-image.html`, which now builds its `srcset`s with the same code the optimizer uses rather than three copies of its own (#335).
- Search results link to the section of a page the query was found in, not just to the page. A result is still one entry per page, with at most three matching sections listed under it, each an anchor to the heading itself; the excerpt now comes from the section's own text. The sections are taken from the page's own `h2` and `h3` at build time, using the ids kramdown already gives them, so there is nothing to write in front matter. A page with no headings indexes as one section, as every page did before; the text before the first heading is never offered as a section of itself; a heading with no id still indexes and links to the page; and the split runs over the document's own content, so the headings a layout puts around an article are not sections of every result. An index generated without sections, by an older build or a site's own template, still searches. The repeated text takes `search.json` from 32 KB to 39 KB served compressed on the demo site, and a budget assertion holds it there (#336).
