...
module Jekyll
  class PublicationsGenerator < Generator
    ...
    def generate(site)
      ...
      combined.each do |entry|
        ...
        citations_map[identifier] = {
          'total' => entry['citations'] ? entry['citations'].to_i : nil,
      ...
      include_bibliography = settings.key?('include_bibliography') ? settings['include_bibliography'] : site.config.dig('theme_options', 'publications', 'include_bibliography')
    ...
    def normalize_entry(entry)
      ...
      if normalized['authors'].is_a?(String)
        normalized['authors'] = parse_authors(normalized['authors'])
      end
    ...
    def compute_metrics(entries, citations_map, override)
      ...
      entries.each do |entry|
        ...
        if citation_value && citation_value.positive?
      ...
      override.each do |key, value|
        if value.is_a?(Hash)
          metrics[key.to_s] = merge_nested_hash(metrics[key.to_s], value)
        else
    ...
    def merge_nested_hash(base, override)
      ...
      override.each do |key, value|
        key = key.to_s
        if value.is_a?(Hash)
          base[key] = merge_nested_hash(base[key], value)
        else
          base[key] = value
        end
      end
